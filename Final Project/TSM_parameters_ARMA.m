...
for j=1:1:10
    ...
    for n = 1:1:10
        ...
        try
            ...
            Mdl = arima('ARLags', (B(2:1+j,2)-ones(length(B(2:1+j,2)),1))','MALags',(B(2:1+n,2)-ones(length(B(2:1+n,2)),1))');
...
Mdl = arima('ARLags', (B(2:row+1,2)-ones(length(B(2:row+1,2)),1))','MALags',(B(2:1+column,2)-ones(length(B(2:1+column,2)),1))');
...
h.Title = 'BIC for ARMA (3R2C model heating energy 60 min)';
h.XLabel = 'MA';
h.YLabel = 'AR';
h.FontName = 'Calibri';
h.FontSize = 24;
...
fileout = [newFilePath 'heatmap_ARMA_3R2C_H_60'];
