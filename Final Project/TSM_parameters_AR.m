%% Read data
str=strcat('E:\U of U\OneDrive - University of Utah\Redisual model\CSC\Matlab Figures and Codes\RD_EP_RC_60m_con.xlsx'); 
data=readtable(str);
...
% 
Mdl = arima('ARLags', (B(2:ind+1,2)-ones(length(B(2:ind+1,2)),1))');
...
% 
% 
% MAE=1/(length(RDC))*sum(abs(RDC-RC_21C));
% MAEf=1/(length(RDC))*sum(abs(RDC-RC_21C_f));
% P1=(MAE-MAEf)/MAE;
% 
% 
% RDC_m=sum(RDC)/length(RDC);
% CVRMSE=sqrt(1/(length(RDC))*sum((RDC-RC_21C).^2))/RDC_m;
% CVRMSEf=sqrt(1/(length(RDC))*sum((RDC-RC_21C_f).^2))/RDC_m;
% P2=(CVRMSE-CVRMSEf)/CVRMSE;
...
% 
% 
% MAE=1/(length(EPC))*sum(abs(EPC-RC_32C));
% MAEf=1/(length(EPC))*sum(abs(EPC-RC_32C_f))
% P1=(MAE-MAEf)/MAE
% 
% 
% EPC_m=sum(EPC)/length(EPC);
% CVRMSE=sqrt(1/(length(EPC))*sum((EPC-RC_32C).^2))/EPC_m;
% CVRMSEf=sqrt(1/(length(EPC))*sum((EPC-RC_32C_f).^2))/EPC_m
% P2=(CVRMSE-CVRMSEf)/CVRMSE
...
% hold on;
% 
% f1 = plot(AIC,'color',C1,'linewidth',2);
% 
% 
% 
% set(gca, 'YColor', 'k',...
...
% 
% hYLabel = ylabel('AIC');
% hXLabel = xlabel('Order of AR model');
% 
% hLegend = legend(f1, ...
%           'Cooling energy by EnergyPLus model (60 min)', ...
%                  'Location', 'northeast');
% 
% 
% 
...
% set([hYLabel,hXLabel], 'FontSize', 24)
% 
% 
% figW = figureWidth;
% figH = figureHeight;
...
% fileout = [newFilePath 'AIC_AR_C_60'];
...
f1 = plot(BIC,'color',C2,'linewidth',2);
...
hLegend = legend(f1, ...
          'Heating energy by EnergyPlus model (15 min)', ...
                 'Location', 'northeast');
  

set(gca, 'FontName', 'Calibri', 'FontSize', 30)
set([hYLabel,hXLabel], 'FontName',  'Calibri')
set([hYLabel,hXLabel], 'FontSize', 30)
...
set(figureHandle,'PaperPosition',[0 0 figW figH]);
newFilePath = 'C:\Users\Zhihao\OneDrive - University of Utah\Redisual model\CSC\Matlab Figures and Codes\AR_order\';
fileout = [newFilePath 'BIC_AR_H_15_EP'];
print(figureHandle,[fileout,'.png'],'-r300','-dpng'); 
